...
- **Frontend**: React 19 + Vite + Tailwind + MapLibre GL. Entry `src/main.jsx`
  ...
  works even if D1 is down. Firebase code in `src/firebase.js` is legacy.
...
python3 dinescores_pipeline.py --mode weekly --cities <slugs...> \
    ...
    --merge-existing-data-js public/data.js --dry-run
```

Pipeline has no third-party deps. `--dry-run` skips Firebase. City slugs
are in the `--cities` dispatch at the bottom of the pipeline file.
